...
let initWS = () => {
  ...
  ws.on('message', (msg) => {
    // console.log('initWS message', msg)
    // 验签工作让应用方做。
    ...
    if (config.autoReceipt) {
      ...
      let receiptmsg = createMessage('', [msgObj.sender], 'receipt', msgObj.messageId)
      ws.send(receiptmsg)
    }
  })
  ws.on('error', (e) => {
...
let send = (content, receiver, method, messageId, createTime) => {
  wsc.send(createMessage(content, receiver, method, messageId, createTime))
}
