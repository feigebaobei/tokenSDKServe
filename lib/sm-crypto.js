...
// const FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc] // 在下面的代码里是直接使用的
 ...
 * @param  {[type]} roundKey [description]
...
function sms4Crypt(input, output, roundKey) {
    ...
    // 32轮加密过程
    // F(x, rk) = x0 ^ T(x1 ^ x2 ^ x3 ^ rk)
    // T(x) = L(r(x))
    // L(b) = b ^ (b <<< 2) ^ (b <<< 10) ^ (b <<< 18) ^ (b <<< 24)
    // r(a) = (a0, a1, a2, a3) => (sbox(a0), sbox(a1), sbox(a2), sbox(a3))
    for(r = 0; r < 32; r += 4) {
 ...
 * @return {[type]}          [description]
 */
function sms4CryptLi(input, output, roundKey, cryptFlag) {
    let r;
    let mid;
    let x = new Array(4);
    let tmp = new Array(4);
    ...
    if (cryptFlag) { // 加密
        for (let i = 0; i < 8; i++) {
            mid = b[1] ^ b[2] ^ b[3] ^ roundKey[31-4*i]
            b[0] = b[0] ^ sbox0[mid&0xff] ^ sbox1[(mid>>8)&0xff] ^ sbox2[(mid>>16)&0xff] ^ sbox3[(mid>>24)&0xff]
            mid = b[0] ^ b[2] ^ b[3] ^ roundKey[31-4*i-1]
            b[1] = b[1] ^ sbox0[mid&0xff] ^ sbox1[(mid>>8)&0xff] ^ sbox2[(mid>>16)&0xff] ^ sbox3[(mid>>24)&0xff]
            mid = b[0] ^ b[1] ^ b[3] ^ roundKey[31-4*i-2]
            b[2] = b[2] ^ sbox0[mid&0xff] ^ sbox1[(mid>>8)&0xff] ^ sbox2[(mid>>16)&0xff] ^ sbox3[(mid>>24)&0xff]
            mid = b[1] ^ b[2] ^ b[0] ^ roundKey[31-4*i-3]
            b[3] = b[3] ^ sbox0[mid&0xff] ^ sbox1[(mid>>8)&0xff] ^ sbox2[(mid>>16)&0xff] ^ sbox3[(mid>>24)&0xff]
        }
    } else { // 解密
    ...
    permuteFinalBlock(output, br)
}
function permuteFinalBlock (output, br) {
    for (let i = 0; i < 4; i++) {
        output[i * 4] = br[i] >> 24
        output[i * 4 + 1] = br[i] >> 16
        output[i * 4 + 2] = br[i] >> 8
        output[i * 4 + 3] = br[i]
    }
}
 ...
 * @param  {[type]} roundKey  [description]
 * @param  {[type]} cryptFlag [description]
 * @return {[type]}           [description]
 */
 ...
 * @return {[type]}           [description]
 */
function sms4KeyExtLi(key, roundKey, cryptFlag) {
    // console.log('src', key)
    let r;
    let mid;
    let x = new Array(4);
...
function sm4(inArray, key, cryptFlag) {
    ...
    // sms4KeyExtLi(key, roundKey, cryptFlag);
...
function sm4Li(inArray, key, cryptFlag) {
    let outArray = [];
    let point = 0;
    let roundKey = new Array(ROUND);
    // sms4KeyExt(key, roundKey, cryptFlag);
    ...
    while (inLen >= BLOCK) {
        ...
        sms4CryptLi(input, output, roundKey, cryptFlag);
...
module.exports = {
    encrypt(inArray, key) {
        ...
        // return sm4Li(inArray, key, 1);
    },
    decrypt(inArray, key) {
        return sm4(inArray, key, 0);
    },
    encryptLi(inArray, key) {
        // return sm4(inArray, key, 1);
        return sm4Li(inArray, key, 1);
    },
};
