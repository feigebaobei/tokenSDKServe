...
(function (global, factory) {
  ...
  function () {
    function Sm4js(config) {
      ...
      // this.key = keyBuffer;
      // this.key = [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10]
      if (config.key.length !== 16) {
        throw new Error('key必须是一个长度为16的数组')
      }
      this.key = config.key
      /**
      ...
      // this.iv = ivBuffer;
      // this.iv = [208, 193, 56, 14, 224, 207, 206, 67, 115, 5, 42, 90, 244, 110, 49, 166]
      // if (config.iv === 'random') {
      ...
      //     return res
      //   }
      //   config.iv = randomArr(0, 255, 16)
      // } else {
      //   if (config.iv.length !== 16) {
      //     throw new Error('vi必须是一个长度为16的数组')
      //   }
      // }
      if (config.iv !== undefined && config.iv !== null) {
        if (config.iv.length !== 16) {
      ...
      this.cipherType = 'base64'
      if (['base64', 'text', 'array'].indexOf(config.cipherType) >= 0) {
        this.cipherType = config.cipherType
      }

      // this.mtType = 'text'
      // if (['array', 'text'].indexOf(config.mtType) >= 0) {
      //   this.mtType = config.mtType
      // }
      this.mtType = ['array', 'text'].indexOf(config.mtType) >= 0 ? config.mtType : 'text'
    ...
    _createClass(Sm4js, [{
      ...
      value: function encrypt(plaintext) {
        ...
        let plainByteArray
        if (plaintext.constructor === Array) {
          plainByteArray = plaintext
        } else {
          plainByteArray = this.Crypt.stringToArrayBufferInUtf8(plaintext);
        }
        // var plainByteArray = this.Crypt.stringToArrayBufferInUtf8(plaintext);
        var padded = this.padding(plainByteArray);
        // padded = [208, 193, 56, 14, 224, 207, 206, 67, 115, 5, 42, 90, 244, 110, 49, 166]
        // padded = [1, 35, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 18, 170, 148, 69, 88, 143, 213, 81, 137, 41, 174, 176, 173, 178, 186, 154, 3, 3, 3] // 112
        // padded = [208, 193, 56, 14, 224, 207, 206, 67, 115, 5, 42, 90, 244, 110, 49, 166, 1, 35, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 18, 170, 148, 69, 88, 143, 213, 81, 137, 41, 174, 176, 173, 178, 186, 154, 3, 3, 3] // 112
        // console.log('origData:', padded)
        ...
        if (this.mode === 'cbc') {
          ...
          for (i = 0; i < blockTimes; i++) {
            ...
            roundIndex = i * UINT8_BLOCK;
            block = this.uint8ToUint32Block(padded, roundIndex);
            cipherBlock = this.doBlockCrypt(block, this.encryptRoundKeys);            

            // for (var l = 0; l < UINT8_BLOCK; l++) {
            for (l = 0; l < UINT8_BLOCK; l++) {
              outArray[roundIndex + l] = cipherBlock[parseInt(l / 4)] >> (3 - l) % 4 * 8 & 0xff;
            }
          }
        } // cipher array to string

        // console.log('加密结果：', outArray)
        ...
        switch (this.cipherType) {
          case 'base64':
            ct = this.Crypt.arrayBufferToBase64(outArray);
            break
          case 'text':
            ct = this.Crypt.utf8ArrayBufferToString(outArray);
            break
          default:
            // ct = outArray
            ct = []
            // let i = 0
            // let iLen = outArray.length
            // while (i < iLen) {
            //   ct.push(outArray[i++])
            // }
      ...
      value: function decrypt(ciphertext) {
        ...
        switch (this.cipherType) {
          ...
          case 'text':
            ...
            break
          default:
            cipherByteArray = ciphertext
            break
        }

        // console.log('cipherByteArray', cipherByteArray)
        ...
        if (this.mode === 'cbc') {
          ...
          for (i = 0; i < blockTimes; i++) {
            ...
            roundIndex = i * UINT8_BLOCK; // make Uint8Array to Uint32Array block

            block = this.uint8ToUint32Block(cipherByteArray, roundIndex); // reverse the round keys to decrypt;

            plainBlock = this.doBlockCrypt(block, this.decryptRoundKeys);
        ...
        // console.log('depaddedPlaintext', depaddedPlaintext.join(' '))

        // return this.Crypt.utf8ArrayBufferToString(depaddedPlaintext);
        let mt
        if (this.mtType === 'array') {
          mt = []
          // let i = 0, iLen = depaddedPlaintext.length;
          // while (i < iLen) {
          //   mt.push()
          // }
          mt = depaddedPlaintext.reduce((res, cur) => {
            res.push(cur)
            return res
          }, [])
        } else {
          mt = this.Crypt.utf8ArrayBufferToString(depaddedPlaintext)
        }
        return mt
