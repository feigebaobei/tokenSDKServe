...
// let priStr = JSON.parse(decryptDidttm(didttm, idpwd).data).prikey
let priStr = JSON.parse(decryptDidttm().data).prikey
 ...
 *                             }
 */
// function decryptDidttm (didttmStr, idpwd, {issm = false, hashKey = true}) {
function decryptDidttm (options) {
  let config = {
    issm: false,
    hashKey: true
  }
  ...
  if (config.issm) {
    ...
    ct = utils.hexStrToArr(ct)
    datamt = sm4.decrypt(ct, idpwd, {hashKey: config.hashKey})
  }
  return {
 ...
 * @param  {String}  options.did     从区块链上得到pvdata时必填did。
 * @return {string | promise}        pvdata的字符串 | promise
 */
function getPvData(options) {
  let config = {
    origin: 'local',
    decrypt: true,
    did: didttm.did
  ...
  switch (config.origin) {
    case 'local':
      ...
      if (config.decrypt) {
        res = decryptPvData(pvdataCt, priStr)
      } else {
        res = pvdataCt
      }
      break
